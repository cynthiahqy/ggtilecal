...
Prepare and reshape event data into "long" form, which in this context refers to having one row per day of an event.
...
```{r empty-calendar-events, fig.height=3, fig.width=9,}
events_long |>
  gg_facet_wrap_months(unit_date)
```
