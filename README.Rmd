...
The goal of `ggtilecal` is to easily produce customisable calendar layouts using ggplot2 tile geoms.
...
Prepare event data in "long" form, which in this context refers to having one row per day of an event.
...
If the emojis are not rendering, try changing your graphics device. For knitr output this can be controlled using the `dev` chunk option. For previews in RStudio, change Settings > General > Graphics (e.g. to [AGG](https://ragg.r-lib.org/)).
